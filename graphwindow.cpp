...
{
    // 绘图部分在窗口上的位置
    float offsetx = ui->txt_graph->pos().x();
    float offsety = ui->txt_graph->pos().y();
    // 绘图部分宽高
    float hei     = ui->txt_graph->height();
    float wid     = ui->txt_graph->width();
    float posx    = 0;
    float posy    = 0;                           // 绘图偏移
    ...
    for (int varIndex = 0; varIndex < varList->size(); varIndex++)
        if (varList->at(varIndex).samples.size() > 0
            && varList->at(varIndex).samples.last().timestamp > totalTime * 1000000)
    ...
    double startTime = ui->bar_hori->value() * 1.0 / ui->bar_hori->maximum()
                     * (totalTime - timeRange);                        // 需显示部分的开始时间
    ...
    for (int varIndex = 0; varIndex < varList->size(); varIndex++)
    {
        ...
        if (!var.enableScope || var.samples.size() < 2) // 若不使能绘制或采样点太少则跳过
            continue;
        painter.setPen(QPen(
            var.lineColor, varIndex == chosenVarIndex ? 4 : 2, Qt::SolidLine)); // 设置笔刷（若是被选中的曲线则加粗）
        ...
        for (int sampIndex = 1; sampIndex < var.samples.size(); sampIndex++) // 依次连接采样点
        {
            ...
            if (lastTime > startTime && lastTime < endTime && thisTime > startTime && thisTime < endTime
                && thisTime - lastTime < 0.5)
            {
                QPoint lastPoint((lastTime - startTime) / timeRange * wid + posx,
                                 hei - (lastSamp.value - minValue) / valueRange * hei + posy);
                QPoint thisPoint((thisTime - startTime) / timeRange * wid + posx,
                                 hei - (thisSamp.value - minValue) / valueRange * hei + posy);
                if (!((lastPoint.y() < posy && thisPoint.y() < posy)
                      || (lastPoint.y() > posy + hei && thisPoint.y() > posy + hei)))
    ...
    if (chosenVarIndex >= 0 && chosenVarIndex < varList->size() && varList->at(chosenVarIndex).samples.size() >= 2)
    {
        ...
        if (looking) // 计算查看位置
        {
            ...
            for (int sampIndex = 0; sampIndex < chosenVar->samples.size() - 1;
                 sampIndex++) // 依次检查采样点找到时间戳与查看位置最接近的点
            {
