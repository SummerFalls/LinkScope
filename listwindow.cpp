...
void ListWindow::parseVarChildren(VarNode &node)
{
    ...
    else if (node.parent->parent == NULL) // 传入的是文件名节点
    {
        ...
        while (true) // 循环查找每一个匹配的文件名
        {
            ...
            QStringList varList = gdb->getVarListFromRawOutput(
                rawVarInfo.mid(startPos, endPos - startPos)); // 截取两个位置之间的子串解析变量列表
    ...
    else                                                                          // 传入的是普通节点
    {
        ...
        else if (!rawVarType.contains('(')) // 不是数组类型，排除函数类型，判定是否为可展开类型
        {
            ...
            if (detailRawVarType.startsWith("struct") || detailRawVarType.startsWith("union")
                || detailRawVarType.startsWith("class")) // 判定为可展开类型
            {
