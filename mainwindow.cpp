    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(ui->action_show_graph,
            &QAction::toggled,
            this,
            [=](bool checked) { graph->setHidden(!checked); }); // 绑定菜单项到窗口开关
    connect(
        graph, &QDialog::rejected, this, [=] { ui->action_show_graph->setChecked(false); }); // 当窗口关闭时取消勾选菜单
...
void MainWindow::slotWatchTimerTrig()
{
    ...
    {
        varList[index].rawValue =
            gdb->captureValueFromDisplay(rawDisplay, varList.at(index).name); // 进行正则匹配，截取出变量值部分
        SamplePoint sample;
        if (gdb->getDoubleFromDisplayValue(varList[index].rawValue,
                                           sample.value)) // 尝试转换为double，生成采样点并写入采样点列表
        {
...
{
    const QString cmd =
        "print/u logQueue.size>0?(logQueue.size--,logQueue.startPos=(logQueue.startPos+1)%logQueue.maxSize,*logQueue.buf[logQueue.startPos]@logQueue.lenBuf[logQueue.startPos]):{0}\r\n";
...
{
    tableModel->disconnect(this,
                           SLOT(slotTableEdit(QModelIndex, QModelIndex))); // 断开信号连接，否则触发后会直接添加到列表
    tableModel->item(tableModel->rowCount() - 1, 0)->setText(name);
    connect(tableModel,
            SIGNAL(dataChanged(QModelIndex, QModelIndex)),
            this,
            SLOT(slotTableEdit(QModelIndex, QModelIndex))); // 恢复信号连接
}
...
void MainWindow::setConnState(bool connect)
{
    if (connect)                        // 进行连接
    {
        ...
        if (ui->rb_openocd->isChecked())          // 选择的是OpenOCD模式
        {
            ...
            {
                openocd->start(ui->cb_interface->currentText(),
                               ui->cb_target->currentText(),
                               configWindowParam.ocdParam,
                               configWindowParam.gdbPort); // 运行openocd进程进行目标连接
                sleep(500);                                // 等待500ms
        ...
        else if (ui->rb_serialocd->isChecked()) // 选择的是串口模式
        {
            ...
            {
                serialocd->startConnect(SerialParam { ui->cb_com->currentText(),
...
void MainWindow::sleep(uint32_t ms)
{
    ...
    while (QTime::currentTime() < untilTime) QCoreApplication::processEvents(QEventLoop::AllEvents, 100);
}
...
void MainWindow::loadConfFileList()
{
    ...
    foreach(QFileInfo fileInfo, interfaceDir.entryInfoList()) ui->cb_interface->addItem(fileInfo.fileName());
    QDir targetDir(QCoreApplication::applicationDirPath() + "/openocd/share/openocd/scripts/target");
    targetDir.setFilter(QDir::Files);
    foreach(QFileInfo fileInfo, targetDir.entryInfoList()) ui->cb_target->addItem(fileInfo.fileName());
}
...
void MainWindow::on_bt_set_axf_clicked()
{
    ...
    fileDialog->setNameFilters(QStringList()
                               << tr("AXF/ELF File (*.axf *.elf)") << tr("所有文件 (*)")); // 设置文件过滤器
...
void MainWindow::initTable()
{
    redrawTable();
    connect(
        tableModel, SIGNAL(dataChanged(QModelIndex, QModelIndex)), this, SLOT(slotTableEdit(QModelIndex, QModelIndex)));
...
void MainWindow::updateGDBList()
{
    ...
    for (int index = 0; index < varList.size(); index++) nameList << varList.at(index).name;
    ...
    if (fileDialog->exec()) // 等待用户选择文件
    {
        ...
        if (file.exists())
        {
            QMessageBox messageBox(
                QMessageBox::NoIcon, "警告", "文件已存在，是否覆盖？", QMessageBox::Yes | QMessageBox::No, NULL);
...
void MainWindow::checkUpdate()
{
                            ...
                            if (tag != APP_VERSION) // 与宏定义中的当前版本号进行比较
                            {
                                ...
                                if (msgBox.exec() == QMessageBox::Yes)
                                {
                                    QDesktopServices::openUrl(QUrl(
                                        "https://gitee.com/skythinker/link-scope/releases")); // 打开仓库release页面
                                }
                            }
                            else
                            {
    ...
    request.setUrl(
        QUrl("https://gitee.com/api/v5/repos/skythinker/link-scope/releases/latest")); // 发送get请求到gitee服务器
    manager->get(request);
}
...
void MainWindow::checkOpenocdProcess()
{
    ...
    if (listProcess.readAllStandardOutput().contains("openocd.exe")) // 后台存在openocd进程
    {
        ...
        msgBox.setWindowTitle("提示");
        msgBox.setText(
            "发现正在运行的OpenOCD进程，可能是由于上次不正常退出导致的，是否强制结束？（若您没有其他正在调试的程序建议结束）");
        msgBox.setStandardButtons(QMessageBox::Yes | QMessageBox::No);
        if (msgBox.exec() == QMessageBox::Yes)
        {
