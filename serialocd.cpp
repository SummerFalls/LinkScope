...
SerialOCD::SerialOCD(QObject *parent)
    : QThread(parent)
{
}
...
void SerialOCD::slotSocketReadyRead()
{
    ...
    else if (input.startsWith("M"))                                                // 写入内存指令
    {
        ...
        for (int i = 0; i < rawData.length() / 2; i++) data.append(rawData.mid(i * 2, 2).toUInt(NULL, 16));
...
void SerialOCD::sendToClient(const QString &data)
{
    ...
    int        sum   = 0; // 进行校验和计算
    for (int i = 0; i < array.length(); i++) sum += array.at(i);
    sum %= 0x100;
